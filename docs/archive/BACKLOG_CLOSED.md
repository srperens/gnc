# GNC Backlog — Closed & Shipped Items (Archive)

Full history of completed, closed, and vetoed backlog items, moved out of
[BACKLOG.md](../../BACKLOG.md) on 2026-09-04 to keep the active backlog readable.
Items are verbatim as they stood at archival. See [RESEARCH_LOG.md](../../RESEARCH_LOG.md)
for the underlying measurements.

Summary of shipped improvements (RS-approved, DONE):
...
- **#35/#40/#41/#43/#44/#45/#46/#47/#48/#50** — CLOSED (gates failed or gain below threshold).
...
- **Status:** CLOSED — gate measurement was never actually run (2026-03-11 finding: "85-88% LL" was analytically inconsistent, never documented as real measurement). Correct closure reason: RS verdict stands — VC-2 achieves H.264-class with wavelet inter, gap is prediction quality not transform mismatch. If detail subbands had been measured on crowd_run, gate would likely PASS (~94% detail energy from count-weighted analysis), but DCT still can't fix poor ME quality — it would only help if block-boundary energy dominates, which OBMC evidence refutes.
...
- **Result (2026-03-11, corrected implementation ffbaf86):** Tile-level skip (256×256) with post-quantize coefficient zeroing — correct domain, no wavelet bleed. **Skip rate: 0/40 tiles on crowd_run and park_joy; 0–2/40 on rush_hour. Bpp effect: negligible.** The fix is architecturally correct but the granularity mismatch is fundamental: H.264 skips 16×16 blocks, we skip 256×256 tiles (256× coarser). Near-static tiles are rare on real content.
- **Root cause of ineffectiveness:** Block-level skip (16×16) is impossible in a tile-wide wavelet codec without a block-independent transform. Wavelet energy from non-skip blocks bleeds into skip-block coefficient positions — decoder cannot reconstruct correctly if those positions are zeroed. True fix requires either (a) DCT for inter residuals (#35) enabling per-block independence, or (b) smaller tile size (infeasible — GPU memory constraint).
- **Closed.** Infrastructure (tile_skip_motion.wgsl output buffer, zero_skip_tiles_by_map.wgsl) left in place but provides no meaningful bpp savings on tested content.
...
- **Team Lead approval:** Bitstream format change approved. Implement full candidate set {8×8, 8×16, 16×8, 16×16, 32×32} — let RD decide which sizes are actually used. The gate showed ~25–40% benefit from sub-16×16; don't limit the candidate set prematurely.
- **Motivation:** Item #40 (forced 4×4 everywhere) failed because MV overhead dominated without RD gating. RD-optimal block size selection — minimize λ·rate + distortion per block — lets the encoder use small blocks at motion boundaries and large blocks on uniform regions. H.264 studies show adaptive partitioning adds ~8–14% BD-rate gain on top of skip mode.
- **Block size candidates:** {8×8, 8×16, 16×8, 16×16, 32×32}. All five. NOT 4×4 (closed by #40).
- **Implementation insight:** No bitstream format change needed — existing 4-sub-block MV structure can implicitly encode all block sizes: 16×16 = all 4 MVs equal, 8×16 = top pair equal + bottom pair equal, 16×8 = left pair equal + right pair equal, 8×8 = all independent, 32×32 = all MVs in a 2×2 macroblock group equal. Decoder needs only to read and use the (already-stored) split MVs as-is.
