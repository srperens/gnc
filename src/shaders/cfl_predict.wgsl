// CfL (Chroma-from-Luma) inverse prediction shader.
//
// Reconstructs chroma wavelet coefficients from the transmitted residual
// and the reconstructed luma reference:
//   output[i] = residual[i] + alpha[tile][subband] * luma_ref[i]
//
// Each thread processes one coefficient. The alpha lookup uses tile index
// and subband index derived from the 2D position, identical to quantize.wgsl.
...
// Determine the flat subband index for a tile-local position (lx, ly).
// Duplicated from quantize.wgsl (WGSL has no include mechanism).
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Alpha lookup: alphas[tile_idx * num_subbands + sb]
    let alpha = alphas[tile_idx * params.num_subbands + sb];

    output[idx] = residual[idx] + alpha * luma_ref[idx];
}
