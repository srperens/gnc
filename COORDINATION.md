...
|---|---|---|
| `../gnc-refdiff` | `refdiff` | **RATE-4 (in flight)** — the two halves RATE-3 left: `0040` point 4's source-copy reference (free, and its refutation is confounded by BUG-39 cause 2, so it may remove RATE-3's third encode outright), and choosing the candidate on *sequence* bytes rather than frame bytes so bbb q=99 stops regressing. Touches `local_decode_iframe_gpu`, which is BUG-39's cause-1 surface — that item's cause 4 is in the sub-pel prediction path, not here. |
