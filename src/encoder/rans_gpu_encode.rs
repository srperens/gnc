    ...
    fn new(ctx: &GpuContext, num_tiles: usize, per_subband: bool) -> Self {
        ...
        Self {
            ...
            hist_buf: ctx.device.create_buffer(&wgpu::BufferDescriptor {
                ...
                // COPY_DST needed for fused quantize+histogram path that copies
                // pre-computed histograms into this buffer.
...
impl GpuRansEncoder {
    pub fn new(ctx: &GpuContext) -> Self {
        ...
        let fused_norm_enc_bgl =
            ctx.device
                .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                    label: Some("rans_fused_ne_bgl"),
                    entries: &[
                        make_uniform_entry(0),        // params
                        make_storage_entry(1, true),  // hist_input
                        make_storage_entry(2, true),  // coeff_input
                        make_storage_entry(3, false), // cumfreq_out
                        make_storage_entry(4, false), // tile_info_out
                        make_storage_entry(5, false), // stream_output
                        make_storage_entry(6, false), // stream_metadata
                    ],
                });
    ...
    /// Split-phase encode: dispatch entropy encode + staging copies to external command encoder.
    /// Call `finish_3planes_readback` after submit+poll to read back and pack tiles.
    ///
    /// This allows the caller to batch entropy encode dispatches with other GPU work
    /// (e.g., local decode) in a single submit, eliminating pipeline stalls.
    ...
    /// Complete the split-phase readback: map staging buffers, poll, and pack tiles.
    /// Must be called after `dispatch_3planes_to_cmd` dispatches have been submitted.
    ...
    pub fn encode_3planes_fused(
        ...
        let mut cmd = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("rans_3plane_fused_cmd"),
            });
        ...
        for (p, quantized_buf) in quantized_bufs.iter().enumerate() {
            ...
            let fused_bg = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("rans_fused_ne_bg"),
                layout: &self.fused_norm_enc_bgl,
                entries: &[
                    wgpu::BindGroupEntry {
                        binding: 0,
                        resource: params_buf.as_entire_binding(),
            ...
            // Pass 2: Fused normalize+encode (256 threads, replaces 2 separate dispatches)
            {
                let mut pass = cmd.begin_compute_pass(&wgpu::ComputePassDescriptor {
        ...
        if std::env::var("GNC_PROFILE").is_ok() {
            eprintln!(
                "[rans_3plane_fused] submit={:.2}ms map_async={:.2}ms poll={:.2}ms recv={:.2}ms pack={:.2}ms total={:.2}ms readback_bytes={:.1}MB",
                t_submit.as_secs_f64() * 1000.0,
                (t_map_async - t_submit).as_secs_f64() * 1000.0,
                (t_poll - t_map_async).as_secs_f64() * 1000.0,
                (t_readback - t_poll).as_secs_f64() * 1000.0,
    ...
    /// Expose internal histogram buffer for use by the fused quantize+histogram shader.
    /// Must call `ensure_buffers` first to guarantee the buffer exists with the right size.
    pub fn ensure_and_get_hist_buf(
        &mut self,
        ctx: &GpuContext,
        num_tiles: usize,
        per_subband: bool,
        ...
        self.ensure_buffers(ctx, num_tiles, per_subband);
        &self.cached.as_ref().unwrap().hist_buf
    }

    /// Batched 3-plane rANS encode with pre-computed histograms.
    ///
    /// Like `encode_3planes_to_tiles` but skips the histogram pass for each plane.
    /// The caller must have already written valid histogram data into the internal
    /// `hist_buf` (via `ensure_and_get_hist_buf`) before calling this for each plane.
    ///
    /// `precomputed_hist_bufs`: For each of the 3 planes, an external histogram buffer
    /// that contains the same data format as rans_histogram.wgsl output.
    /// The method copies each plane's histogram into the internal hist_buf before
    /// running normalize + encode.
    pub fn encode_3planes_skip_histogram(
        &mut self,
        ctx: &GpuContext,
        quantized_bufs: [&wgpu::Buffer; 3],
        hist_bufs: [&wgpu::Buffer; 3],
        info: &FrameInfo,
        per_subband: bool,
        num_levels: u32,
    ) -> (Vec<InterleavedRansTile>, Vec<SubbandRansTile>) {
        ...
        let mut cmd = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("rans_3plane_skip_hist_cmd"),
            });
            ...
            .enumerate()
        {
            ...
            // Skip histogram pass — hist_buf already populated by fused shader + copy
