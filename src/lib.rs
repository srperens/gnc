...
pub struct FrameInfo {
    ...
    /// Round the padded plane up to this, not to `tile_size`. 32 since GP20 (TILE-1);
    /// equal to `tile_size` for GP18 and older so those files keep their original grid.
    pub plane_pad_align: u32,
}

impl FrameInfo {
    /// Current encoder grid (GP20 / TILE-1 stage 1).
    pub fn new(
        width: u32,
        height: u32,
        bit_depth: u32,
    ...
    /// Padded plane width (rounded up to `plane_pad_align`, 32 since GP20).
