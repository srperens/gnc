...
## There are TWO Macs, and a Mac row must say which one (2026-09-14, answered by the owner)

**Answered the same day it was asked.** The owner runs this repository on **two** Macs:

| | `gnc gpu-info` prints | memory |
|---|---|---|
| the machine CLAUDE.md's Platform Notes describe | `Apple M5 Pro` — 20 GPU cores, 18 CPU cores | 64 GB |
| the machine PERF-4's numbers were taken on | `Apple M1 Pro` — 16 cores | 16 GB |

So the Platform Notes are **not wrong**, and neither was the 2026-09-08 `--density-still` round
that reports 8/8 complete "on 64 GB against a laptop's RAM". They are simply about a different
machine from the one this session ran on, and nothing in the tree said so.

**The rule that follows is the one BUG-29 has been asking for: a throughput row names its machine,
and it names it from `gnc gpu-info`, not from CLAUDE.md.** "On the Mac" is not a label — there are
two, and their GPU core counts differ by 25% and their memory by 4x. A row without a machine is a
row that cannot be compared to anything, and the density and latency rows are exactly where that
bites: MEAS-6's 25.2 ms round trip, MEAS-5's Mac density sweep and PERF-4/`0085`'s 70-90 Mpixel/s
ceiling are **three different questions on at least two different machines**.

**Do not edit the Platform Notes to match whichever machine you are on.** They describe the M5 Pro
and that is correct; add your own machine to your row instead. `gnc gpu-info` is the authority for
the machine in front of you, and CLAUDE.md's Platform Notes say as much themselves.
...
|---|---|---|
| `.claude/worktrees/perf4` | `worktree-perf4` (pushed) | **PERF-4 ANSWERED and rejected 2026-09-14** (`0085`). Built `gnc density` — N encode streams as threads in one process, timed on the steady state behind a barrier — and the prescription it was filed to test fails: **removing every per-process cost did not raise the ceiling** (1.31x shared device, 1.57x device-per-stream, against the process sweep's 1.69x), and **sharing the device is worse than not sharing it**, because `device.poll(Maintain::Wait)` in the readback path is device-wide (~40 call sites; filed **PERF-5**, P1, whose real prize is the 36% of this GPU idle while *one* stream runs). **The ceiling is a pixel rate** — 70.5 / 70.8 / 88.9 Mpixel/s at 720p / 1080p / 4K against fps ceilings that move 7x — so it is the GPU, and **MEAS-5's Claim B should be stated in Mpixel/s**. **Invalidates `--density-still`'s scaling column as a concurrency measurement**: its N=1 reads 7.27 fps where steady state is 23.4, so the column is substantially setup amortisation (noted on MEAS-5). Two of PERF-4's premises did not reproduce: pipeline build is **24.8 ms** warm (1 987 ms cold, which is the '15 s per instance'), RSS **230 MB**/process not 1.8 GB. Shipped `gnc density`, `gpu_tier_bench.py --density-inproc`, `tests/concurrent_streams.rs`. **Touches `main.rs` (new subcommand), `pipeline.rs` (one `clippy::nonminimal_bool` simplification that was blocking the native gate — `codec-fingerprint 6a9fa6bd` unchanged either side), `scripts/gpu_tier_bench.py`.** Filed **BUG-58**: 8K cannot encode at all, one 398 MB buffer against the 256 MiB GNC deliberately requests. **Machine: Apple M1 Pro / 16 cores / 16 GB** — the owner confirmed there are **two Macs** (the Platform Notes describe the other, M5 Pro / 20 cores / 64 GB), so a Mac row must name its machine from `gnc gpu-info`; see the section above. That answer filed **MEAS-15 (P1)**: the second Mac is a 25% bigger GPU already in hand, so MEAS-5's Claim B, restated in Mpixel/s, is one command on it rather than a discrete NVIDIA card. |
...
| `../gnc-intra2` | `intra2` | **Merged after INTER-2, and the resolution mattered.** INTER-2 landed mid-verification and touches the same three lines. Taking its line verbatim — `config.dead_zone * inter_dead_zone_mul()` — would have handed the inter path a **0.6** dead zone at q=85..95, where INTER-2 priced the ladder's 0.179 and 0.0125: its conclusion was measured where `config.dead_zone` *was* the ladder value, which is now `dead_zone_referenced`. The merge keeps its refactor and this change's source, and **every sequence is byte-identical to `main` afterwards**. Also: INTER-2's record took **0041**, reserved here through `scripts/claim` at 13:38:33Z before the file was written — sixth collision, same asymmetry as `0024`/`0027`, so the unreserved record moved to **0043** with its five inbound references updated. |
| `../gnc-intra2` | `intra2` | **INTRA-2 DONE 2026-09-08 — 90% of its blocker was a knob that moved two things.** 0028 could not ship the dead zone because worst-frame went negative on 9 of 9 sequence points; but `res_dead_zone = config.dead_zone * inter_dz_mul` with the multiplier at **2.0**, so `GNC_DEAD_ZONE=0.6` took the *inter* dead zone from the ladder's 0.357/0.025 at q=90/95 — **no-ops** — to 1.2. Holding inter at the ladder's value: worst-frame **0.00/+0.03 dB at q=85**, −0.12…−0.19 at q=90/95. Shipped as `dead_zone` floored at 0.6 over **q=85..95 only**, plus `dead_zone_referenced` for the two places that must not see it: a **referenced** I-frame and the inter path. The gate is on *being referenced*, not on being intra — the residue is arithmetic (at ki=9 the I-frame is 1 of 16, so its saving dilutes 16:1 while worst-frame is fully exposed, because **the worst frame is the I-frame**). Same conclusion and adjacent line to PAD-1's `pad_fill_decay` (`0039`). **Criterion met on all three clauses: stills BD-rate −5.01%, sequences byte-identical, dE00 −5.2% at matched rate with Y-PSNR +0.39 dB.** q=100 bit-exact, VMAF −0.01, **BASELINE q=90 -> 49.89 dB / 7.21 bpp / 97.06**. Decision `0041`. **For INTER-2, in flight:** removing the inter dead zone entirely measured **+2.5 dB of worst-frame at q=85** here, and your "the inter dead zone is the intra one" was taken where `config.dead_zone` was the ladder value — that is now `dead_zone_referenced`, which is what `res_dead_zone` multiplies. |
| `../gnc-doc2` | `doc2` | **DOC-2 — GOALS's target table was stale on the rows that set every session's priority. FIXED 2026-09-08.** Four rows and the ordering sentence: 10-bit called "not implemented" when FMT-1 shipped it 2026-09-06 (**verified here: q=100 bit-exact at 10 bits, max error 0 over 6 220 800 samples**), latency called "never measured" when MEAS-6 measured ~80 ms twice, the intra VMAF figure quoted uncaveated when BASELINE says it predates the high-q ladder fix, and *"the two metrics … come before further compression work"* — which had stopped meaning anything, since one is measured and the other is parked on **hardware, not effort**. **The drift direction is the lesson: README was already right about all of it** (DOC-1 kept it current), so the public document was fine and the internal source of truth was two days behind — the more expensive way round, because work is picked from the internal one. Also **retitled ENT-3**, whose heading asked a question its own body answers with nine measured points; not closed, because q=95-99, context retuning and the unrecorded frame mix are genuinely open. **Documentation only; no code, no shader, no bitstream, no figure moved.** |
| `../gnc-bugfix` | `bugfix` | **BUG-16 FIXED 2026-09-08 — the fused quantiser had a dead zone the other two did not.** Phase 1.5 of `quantize_histogram_fused.wgsl` re-zeroed ±1 values in subband groups already >=95% zero; `dz_mul` appears 7 times there and **0 times in `quantize.wgsl`** or the CPU quantiser, so which coefficients an encode produced depended on which quantiser ran. **All three rows of BUG-16's table are that one feature**, including the 4:2:2 row: GPU vs CPU decoded pixels now differ by **0**, where it recorded 1.69. Priced GPU-against-GPU with a new `flags` bit (comparing against `--cpu-encode` cannot price it — that arm is independently worse): **BD-rate +1.02% mean, direction-inconsistent**, so **off by default**, kept behind `GNC_SPARSE_DZ=1`. Decision `0038`. **Only q <= 30 is affected** — byte-identical at 40/50/75/85/90/100 — so **BASELINE's q=25 row moved to 35.63 dB / 1.64 bpp / VMAF 90.31** (VMAF **+0.06**, an improvement) and the other three rows did not need re-measuring. GOALS's copy updated. **A third test this session had to be inverted** because it asserted the defect as expected behaviour. Left for BUG-28's owner: `abac_handles_subsampled_chroma`'s CPU pin is probably now unnecessary. |
| `../gnc-bugfix` | `bugfix` | **BUG-24, BUG-23 and BUG-22 FIXED 2026-09-08 — working through "everything broken".** BUG-24: the wasm clippy gate was red on `main` and it was the *CLI*; the binary now carries `required-features = ["cli"]` so it is not built for a target it cannot compile for, and CLAUDE.md's gate reads `--lib`, matching LOOP.md. BUG-23 + BUG-22 are **one configuration failing twice**: `--huffman -q 100 -t 512` hung (8 min), then refused, then overflowed its stream slot, and now **encodes in 0.63 s and decodes bit-exact lossless (max error 0)**. Length limiting is now a real tree on halved frequencies — terminating, and Kraft-complete by construction, which the old redistribution was not — and the stream slot is sized from `symbols_per_stream` at four bytes per symbol with **both** shader writes bounded. **The q=90 tile-512 arm BUG-22 measured at 7.8-10.9 dB now reads 49.57-50.08 dB on all four stills, against BASELINE's 50.06** — about +40 dB. **Invalidates no measurement: Rice q=90 is byte-identical** to the pre-session baseline; only Huffman files and `Cargo.toml` changed. 239 tests, both clippy targets clean. Decision `0037`. |
...
- **ARCH-3 + BUG-18 — one P/B frame encoder, and `gpu_entropy_encode` stopped choosing one.**
  ...
  **New item filed on the way out, BUG-28 — filed as BUG-26, renumbered (P2):** abac and Rice decode to *different pixels* at
  ...
  identical pixels" and decision `0055`'s "every chroma format at once" are 4:4:4 claims, and
...
- **BUG-27 — the encoder's P-frame reference was dequantised with the *intra* quantiser step, so
  ...
  **MEAS-3's +4.6% mean / +19.1% worst-frame** (ladder q=25-95) and **decision 0056**, which rests
...
two of three sequences. Decision record 0056; the follow-up is BACKLOG **INTER-1**, unclaimed.
