...
## This machine says Apple M1 Pro / 16 GB; CLAUDE.md says M5 Pro / 64 GB (2026-09-14, perf4)

**`gnc gpu-info` here prints `Apple M1 Pro [Metal, IntegratedGpu]`, `sysctl hw.memsize` reports
16 GB, and `system_profiler` reports 16 cores.** CLAUDE.md's Platform Notes say *Apple M5 Pro — 20
GPU cores, 18 CPU cores, 64 GB, Metal 4*, and BUG-29 already records that the changeover date is
written down nowhere.

**So a Mac throughput row is not automatically comparable to another Mac throughput row.** The
2026-09-08 `--density-still` round reports 8/8 instances complete with the Windows box OOM'ing at
N=8 "on 64 GB against a laptop's RAM" — that is either a different machine from this one or a
mislabelled note, and there is no way to tell from the tree. PERF-4 labelled every number it took
with what `gpu-info` printed rather than with the CLAUDE.md line, and `0085` says so explicitly.

**Do the same, and do not edit the Platform Notes to match whichever machine you are on** — that
line has already been wrong for an unknown length of time in the other direction, and flipping it
without knowing how many machines there are would just re-create BUG-29 pointing the other way.
`gnc gpu-info` is the authority for the machine in front of you; CLAUDE.md's own Platform Notes
say as much. **This is a question for the owner**: how many machines run this repository, and
which one produced the M5 Pro rows?
